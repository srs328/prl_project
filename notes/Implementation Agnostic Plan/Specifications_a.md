...
The LST-AI lesion mask is expected at:

```bash
${subject_root}/lst-ai/space-flair_seg-lst.nii.gz
```

If this file exists, it should be copied or linked into the pipeline’s working data structure. 

It should definitely exist for any subject who has ground truth segmentations or labeled PRL because the PRL labels correspond to the index in ${subject_root}/lst-ai/lstai_lesion_index.nii.gz. If not, an error should be raised

If it does not exist for subjects who just have counts only, the subject should be recorded in a missing-LST-AI report. The pipeline should be designed so that LST-AI could be run automatically in the future, but it is acceptable for the first implementation to only detect and report missing LST-AI outputs.

The pipeline should not assume that all subjects have manual PRL annotations.

---

# Subject category semantics

The pipeline needs to distinguish at least three subject/data categories. The three subject categories are not interchangeable.

## 1. Full ground-truth subjects

These are the only subjects that should be used for voxelwise segmentation training/evaluation by default. They have usable manual PRL segmentation files and enough label information to construct both PRL and non-PRL lesion examples.
...
These subjects are mainly useful for:
...
These subjects may have MRI, LST-AI lesion segmentations, and spreadsheet-level PRL counts, but no voxelwise PRL segmentation labels. Their subid's are recorded in `subjects_with_counts_only.txt`. 

To rephrase, these subjects have subject-level PRL counts but no reliable lesion-index mapping and no voxelwise labels. Any lesion indices in their spreadsheet rows (see next section) should be ignored as stale/outdated.
...
They cannot be used for voxelwise segmentation metrics unless ground truth is later added. They can be used for inference and subject-level predicted-count comparisons, but not voxelwise metrics or lesionwise supervised training.

## Supplementary resource files

The implementation may assume the following files are provided in the project resources directory.
...
sub is the numeric subject ID without the sub-ms prefix.
ses is the numeric session date/id used in ses-{ses}.

Original data path:

`${ORIG_DATAROOT}/sub-ms${sub}/ses-${ses}`

Working subject identifiers should preserve both subject and session.

`subjects_with_groundtruth.txt`

One numeric subject ID per line. These subjects have full voxelwise PRL ground truth.

`subjects_with_labeled_prls.txt`

One numeric subject ID per line. These subjects have PRL labels/count/index metadata but should not automatically be treated as full segmentation-training subjects.

`subjects_with_counts_only.txt`

One numeric subject ID per line. These subjects have counts only. Lesion index columns should be ignored for these subjects.

`PRL_spreadsheet-lstai_update_label_reference.csv`

Used for subjects with ground-truth or labeled PRLs. Important columns:
...
PRL<i>_label gives the lesion index in lstai_lesion_index.nii.gz.
confidence.<i-1> gives the confidence for PRL<i>_label.

For now, only definite and probable should be considered positive PRLs. possible should be excluded by default, but the threshold should be configurable.

`PRL_labels_master_full.csv`

Contains all subject categories. For count-only subjects, the PRL column is the useful count field. Total PRL may be empty for these rows and should not be interpreted as absence of PRLs.
...
This tells which manual segmentation suffix to use for training labels for each ground-truth subject.


### 3. Clarify the two different subject path conventions

Your spec contains both:

```bash
${ORIG_DATAROOT}/sub-ms${subid}/ses-${sesid}
```

and a working folder style like:

sub{subid}-{date_mri}

That is fine, but Codex should know one is source and one is a possible working representation. Add:

## Source paths vs working paths

The original data root uses:

```text
${ORIG_DATAROOT}/sub-ms${subid}/ses-${sesid}

The implementation may choose its own working-data layout, but every staged subject must retain a stable mapping back to:

subid
sesid
original_subject_root

A convenient working subject ID is:

sub{subid}-{sesid}

but this is a convention, not a property of the original data root.
