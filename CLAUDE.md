...
i'm finally preparing for a gridsearch. So I want you to first get familiar with the code that is generated by Auto3dSeg. Why don't you start by taking a close look at the auto3dseg directories. My latest run is in `$PRL_TRAIN_ROOT/roi_train2/run3`. What I can tell you to start is that the main model training logic is contained in `segresnet_<ifold>/scripts/segmenter.py`. But I want you to look at the files in `run3/algorithm_templates/segresnet/scripts/` because I don't want to mess around with modifying segmenter.py: I should set upstream parameters in such a way that the generated segmenter.py will do what I want. 

I believe that with segresnet, anything in the `train_param` dict that I pass to `runner.set_training_params` in `training/roi_train2/my_autorunner.py` will be added to `segresnet_<ifold>/config/hyper_parameters.yaml`. And that goes into the `config` variable in segmenter.py. So I've been looking through the script to find hyperparameters that would be useful for me to edit. If you look at `training/roi_train2/monai_config.jsonc`, you'll see a few that I've identified. For the moment, I have them set to whatever was default generated for run3. 